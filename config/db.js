...
class ConnectDB {
  ...
  async handleDisconnect() {
    ...
    console.log(
      `🔄 Retrying connection in ${delay / 1000} seconds... (${this.retryCount}/${this.maxRetries})`
    );
  ...
  async createIndexes() {
    try {
      ...
      if (models.Order) {
        await models.Order.collection.createIndexes([
          { key: { userId: 1, createdAt: -1 } },
          { key: { orderId: 1 }, unique: true },
          { key: { status: 1 } },
        ]);
      ...
      if (models.Product) {
        await models.Product.collection.createIndexes([
          { key: { category: 1 } },
          { key: { price: 1 } },
          { key: { isActive: 1 } },
        ]);
