class SMSTemplates {
  ...
  static orderCreated(order) {
    return `Hi ${user.firstName || "Customer"}, your order #${
      order.orderId
    } has been received. We'll notify you once it's confirmed.`;
  }

  static orderConfirmed(order) {
    return `Great news! Your order #${
      order.orderId
  ...
  static orderOutForDelivery(order) {
    ...
    } is out for delivery! Our delivery partner will reach you soon.${
      order.deliveryContact ? ` Contact: ${order.deliveryContact}.` : ""
    }`;
  }
  ...
  static subscriptionCreated(subscription) {
    ...
    return `Hi ${
      subscription.userName || "Customer"
    }, your ${planName} subscription is now active. Next delivery: ${nextDeliveryDate}.`;
  }
  ...
  static subscriptionCancelled(subscription) {
    const planName =
      subscription.planName || subscription.plan?.name || "your subscription";
  ...
  static walletDebit(transaction) {
    ...
    const newBalance = transaction.newBalance || transaction.balance || 0;
    const description =
      transaction.description || transaction.purpose || "for payment";
  ...
  static deliveryScheduled(delivery) {
    ...
    return `Delivery scheduled for ${deliveryDate}.${
  ...
  static deliveryCompleted(delivery) {
    const deliveryId =
      delivery.deliveryId || delivery._id?.toString().slice(-8) || "";
  ...
  static newOrderAlert(order) {
    const orderId =
      order.orderId || order.orderNumber || order._id?.toString().slice(-8);
    const customerName =
      order.userName || order.user?.firstName || "A customer";

    return `📦 New Order #${orderId} from ${customerName}. Amount: ₦${
      order.totalAmount || 0
    }`;
  }
  ...
  static newSubscriptionAlert(subscription) {
    const planName =
      subscription.planName || subscription.plan?.name || "new subscription";
    const customerName =
      subscription.userName || subscription.user?.firstName || "A customer";
  ...
  static newSupportTicketAlert(ticket) {
    const ticketNumber =
      ticket.ticketNumber || ticket._id?.toString().slice(-8);
    const customerName =
      ticket.userName || ticket.user?.firstName || "A customer";
  ...
  static specialOffer(offer) {
    ...
    return `${title}${discount ? `: ${discount}` : ""}${
  ...
  static birthdayWish(user) {
    ...
    }! Enjoy a special gift from us on your special day!`;
  }
  ...
  static feedbackThankYou(user) {
    return `Thank you ${
      user.firstName || "Customer"
    } for your valuable feedback! We appreciate you helping us improve.`;
  }
